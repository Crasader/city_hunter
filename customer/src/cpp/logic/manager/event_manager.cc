...
EventManager* EventManager::getInstance()
{
    ...
    //static EventManager& s_event_manager;
    //if( !s_event_manager.init() )
    //{
    //    //SAFE_DELETE(*s_event_manager);
    //    //CCLOG("event_manager init failed!");
    //}
                                           ...
                                           bool cleanup)
{
    ...
    for (auto it = id_listener_map_.begin();it != id_listener_map_.end();++it)
    {
        std::vector<EventListener*>::iterator iter = std::find_if(
            it->second.begin(), 
            it->second.end(), 
    ...
    if ( !listeners_to_add_vec_.empty() )
    {
        std::sort(listeners_to_add_vec_.begin(), 
            listeners_to_add_vec_.end());
        std::vector<EventListener*>::iterator iter = std::find_if(
            listeners_to_add_vec_.begin(), 
            listeners_to_add_vec_.end(), 
...
{
	EventIDListenerMap::iterator it = id_listener_map_.find(event_id);
	if (id_listener_map_.end() != it)
	{
